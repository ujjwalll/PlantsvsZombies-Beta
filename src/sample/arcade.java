...
public class arcade extends Application {
    ...
    String hope;
    private volatile ArrayList<Projectile> pea_proj = new ArrayList<>();
    private volatile Boolean Game_won=false;
    private volatile ArrayList<Zombies> z;
    private volatile ArrayList<Ranged> Pea_shooter;
    private volatile ArrayList<Sunflower> sunflowers = new ArrayList<>();
    private volatile ArrayList<Plants> rest_plants = new ArrayList<>();
    private volatile ArrayList<ImageView> pea = new ArrayList<>();
    private volatile ArrayList<Integer> arr = new ArrayList<>();
    private volatile ArrayList<ImageView> lawnmovers = new ArrayList<>();
    ...
    final Boolean sun_flower_availability =false;
    final Boolean wallnut_availability =false;
    final Boolean cherry_bomb_availability =false;

    public Boolean getGame_won() {
        return Game_won;
    }

    public arcade(User u){
        z=u.getZombies();
        Pea_shooter=u.getPeaShooter();
        sunflowers =u.getSunflowers();
        rest_plants=u.getRest_plants();
        lawnmovers =u.getLawnmovers();
    }
    public arcade(){
        for (int i=0;i<5;i++){
...
//        gp.setMaxWidth(628.0);
        ...
        for (int i = 0; i < rest_plants.size(); i++) {
            root.getChildren().add(rest_plants.get(i).getNormal_imageview());
        }
        for (int i = 0; i < sunflowers.size(); i++) {
            root.getChildren().add(sunflowers.get(i).getNormal_imageview());
        }
        for (int i = 0; i < lawnmovers.size(); i++) {
            root.getChildren().add(lawnmovers.get(i));
        }
        ...
        timeline_check_winner.setCycleCount(100);
